...
jsPsych.plugins['categorize-image-asp2'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('categorize-image-asp2', 'stimulus', 'image');

  plugin.info = {
    name: 'categorize-image-asp2',
    description: '',
    parameters: {
      stimulus: {
      ...
      sentence_stimuli: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'sentence_stimuli to be displayed',
        default: null,
        description: 'sentence_stimuli'
      },
      typing_duration: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // display avatar 
    if (trial.avatar !== undefined) {
      display_element.innerHTML = '<div class="avatar"><img id="avatar" src="'+trial.avatar+'"></img></div>';
    }

    // display typing effect 
    if (trial.typing != undefined) {
      display_element.innerHTML += '<div class="typing"><img id="typing" src="'+trial.typing+'"></img></div>';
    }

    // display typing prompt typing_prompt
    if (trial.typing_prompt != null) {
      display_element.innerHTML += trial.typing_prompt;
    }

    // if sentence_stimuli is set, show sentence_stimuli
    if (trial.sentence_stimuli !== null) {
      display_element.innerHTML += trial.sentence_stimuli;
      display_element.querySelector('#prime_sentence').style.visibility = 'hidden';

      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#prime_sentence').style.visibility = 'visible';
